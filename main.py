...
def get_ghiblog():
    global ghiblog
    ghiblog = user.get_repo('%s/issues' % user.get_user().login)
...
def bundle_summary_section():
    ...
    labels_html_url = 'https://github.com/%s/issues/labels' % user.get_user().login
    issues_html_url = 'https://github.com/%s/issues/issues' % user.get_user().login
...
def format_issue_with_labels(issue: Issue):
    ...
    for label in labels:
        labels_str += sub('[%s](https://github.com/%s/issues/labels/%s)\t|\t' % (
